...
import 'package:task_manager_app/ui/controllers/forgot_password_verify_otp_provider.dart';
...
class _ForgotPasswordVerifyOtpScreen
  ...
  final ForgotPasswordVerifyOtpProvider forgotPVOtpProvider =
              ...
              child: Column(
                ...
                children: [
                  ...
                  Consumer<ForgotPasswordVerifyOtpProvider>(
                    builder: (context, forgotPVOtpProvider, _) {
                      return Visibility(
                        visible:
                            forgotPVOtpProvider.verifyOtpInProgress == false,
  ...
  Future<void> _recoverVerifyOtp(String email, String otp) async {
    bool isSuccess = await forgotPVOtpProvider.recoverVerifyOtp(email, otp);
    if (isSuccess) {
      showSnackBarMessage(context, forgotPVOtpProvider.message ?? '');
      _onTapVerifyButton();
      setState(() {});
    } else {
      showSnackBarMessage(
        context,
        forgotPVOtpProvider.message ?? 'something went wrong',
      );
    }
  }
